...
class FeatureEngineer:
    ...
    def handle_missing_values(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        required_variasi_cols = ["nama_produk", "harga_awal", "nama_variasi"]
        if all(col in df.columns for col in required_variasi_cols):
            valid_variasi_df = df[df["nama_variasi"].notna()]

            product_price_mode = (
                valid_variasi_df.groupby(["nama_produk", "harga_awal"])["nama_variasi"]
                .agg(lambda x: x.mode().iloc[0] if not x.mode().empty else np.nan)
                .to_dict()
            )

            product_mode = (
                valid_variasi_df.groupby("nama_produk")["nama_variasi"]
                .agg(lambda x: x.mode().iloc[0] if not x.mode().empty else np.nan)
                .to_dict()
            )

            df["nama_variasi"] = df.apply(
                self._fill_variasi, args=(product_price_mode, product_mode), axis=1
            )

        # Imputasi Harga Awal Berdasarkan Median
        if "harga_awal" in df.columns:
            global_median = df["harga_awal"].median()
            global_median = 0 if pd.isna(global_median) else global_median

            product_variasi_median = (
                df.groupby(["nama_produk", "nama_variasi"])["harga_awal"].median().to_dict()
            )
            product_median = df.groupby("nama_produk")["harga_awal"].median().to_dict()

            df["harga_awal"] = df.apply(
                self._fill_harga,
                args=(product_variasi_median, product_median, global_median),
                axis=1,
            )

        # Harga Diskon Fallback ke Harga Awal
        if "harga_diskon" in df.columns:
            df["harga_diskon"] = pd.to_numeric(df["harga_diskon"], errors="coerce").fillna(
                df["harga_awal"]
            )
        ...
        df["returned_quantity"] = pd.to_numeric(df["returned_quantity"], errors="coerce").fillna(0)
